<template>
<div>
    <div v-for = "link in links" v-bind:key = "link.SortOrder" 
    class="flex-container_1">
        <div><br></div>
            
            <!-- this is the head -->
            <div v-if="link.Category === 'Header Content'">
            <div class="flex-container_3A" style="text-align: center"
                :style="{ 'backgroundColor': link.Week_Card_Color }">
            <h2>{{ link.SectionTitle }} <br>
            {{ link.Week }} </h2>
            <div v-if="link.SectionSubTitle"><h3> {{ link.SectionSubTitle }} </h3></div>
            <div v-if="link.Text"><p> {{ link.Text }} </p></div>
            <div v-if="link.Graphic">
                <div class="flex-item_1">
                    <a v-bind:href="link.Graphic_Link + '&ref=Kids_Break_Header_Graphic_Link_' + link.Week"
                        target="_blank">
